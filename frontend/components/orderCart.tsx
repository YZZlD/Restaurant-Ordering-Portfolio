...
export default function OrderCart({cart, modifyTotal, total, removeLastItem}: {cart:any[], modifyTotal:Function, total:number, removeLastItem:Function})
{
    return (
        <div className="sticky top-40 m-10 h-1/2 col-start-7 col-span-4 bg-gray-300 border-4 border-black rounded-md overflow-y-auto space-y-4">
                ...
                {cart.map(item => (
                    <div key={item.itemInfo.id} className="px-2">
                        <CartItemCard removeLastItem={(id:number) => removeLastItem(id)} cartItem={item} modifyTotal={(value:number) => modifyTotal(value)} />
                    </div>
                ))}
